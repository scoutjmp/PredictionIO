...
APPDATA_DIRNAME = "PredictionIO/experimental/realtime/ml-100k"
...
class RateAction:
  def __init__(self, uid, iid, rating, t, epoch_t):
    self.uid = uid
    self.iid = iid
    self.rating = rating
    self.t = t
  ...
  def __str__(self):
    return "RateAction[uid=%s,iid=%s,rating=%s,t=%s,e=%s]" % (
        self.uid, self.iid, self.rating, self.t, self.epoch_t)
...
class AppData:
  ...
  def __init_rate_actions(self):
    ...
    for line in f:
      ...
      epoch_t = int(data[3])
      t = datetime.datetime.utcfromtimestamp(epoch_t).isoformat()
      self.add_rate_action(RateAction(data[0], data[1], data[2], t, epoch_t))
    f.close()
